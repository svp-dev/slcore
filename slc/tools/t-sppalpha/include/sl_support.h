...
[[#]]define __sl_prologue(Name, GI, SI, GF, SF, IDXREG, LN) \\
  ...
  void __slf_ ## Name (void) {						\\
      register long __sl_index_ __asm__(IDXREG);			\\
      __asm__ ("# MT: index starts in %0 (must be =" IDXREG ")" : "=r"(__sl_index_)); \\
      register const long __sl_index = __sl_index_;                     \\
...
[[#]]define __sl_declglp_r(Type, Name, Reg) \\
  register Type __sl_glparmi_ ## Name __asm__(Reg); \\
  __asm__ __volatile__("# MT: global init %0" : "=rf"(__sl_glparmi_ ## Name)); \\
  register Type const __sl_glparm_ ## Name = __sl_glparmi_ ## Name

[[#]]define __sl_declglp_w(Type, Name, Reg) \\
  register Type __sl_glparmr_ ## Name __asm__(Reg);	\\
  __asm__ __volatile__("# MT: global init %0" : "=rf"(__sl_glparmr_ ## Name)); \\
